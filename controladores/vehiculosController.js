...
async function listar(req, res) {
  ...
  try {
    let resultado;
    if (cliente_id) {
      resultado = await pool.query('SELECT * FROM vehiculos WHERE cliente_id = $1 ORDER BY id', [cliente_id]);
    } else {
      resultado = await pool.query('SELECT * FROM vehiculos ORDER BY id');
    }
...
async function historial(req, res) {
  try {
    const resultado = await pool.query(
      `SELECT o.id AS orden_id, o.fecha_ingreso, o.fecha_entrega_real, o.estado, o.total,
...
async function crear(req, res) {
  ...
  try {
    const resultado = await pool.query(
      ...
      [cliente_id, placa, marca, modelo, anio, color, kilometraje || 0]
    );
...
async function eliminar(req, res) {
  try {
    const resultado = await pool.query('DELETE FROM vehiculos WHERE id = $1 RETURNING id', [req.params.id]);
    if (resultado.rows.length === 0) {
      return res.status(404).json({ error: 'Vehículo no encontrado' });
    }
